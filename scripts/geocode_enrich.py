...
class GeocodeCache:
    ...
    def get(self, key):
        cur = self.conn.execute(
            "SELECT latitude, longitude, source, confidence FROM geocode_cache WHERE address_key = ?", (key,)
        )
        row = cur.fetchone()
        if row is None:
            return None
        return {"latitude": row[0], "longitude": row[1], "source": row[2], "confidence": row[3]}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.warning(
                    "Census batch attempt %d/%d failed (%s); retrying in %ds", attempt, max_retries, error, delay
                )
                time.sleep(delay)
    raise last_error
...
def _apply_cached(record, cached):
    ...
    if cached.get("latitude") and cached.get("longitude") and not geocoding.has_coordinates(record):
...
def enrich_records(records, cache, args):
    ...
    counters["dedup_saved"] = sum(len(key_to_indices[k]) for k in unique_keys) - len(unique_keys)

    if args.dry_run or not unique_keys:
        if args.dry_run:
            logger.info("Dry run: would query %d unique address(es).", len(unique_keys))
        return counters
    ...
    for chunk_number, chunk in enumerate(_chunks(query_rows, args.batch_size), 1):
        ...
        try:
            response_rows = _post_batch(chunk, args.benchmark, args.timeout, args.max_retries)
        except requests.RequestException as error:
            logger.error(
                "Census batch %d/%d failed permanently: %s; leaving these records unresolved",
                chunk_number,
                total_chunks,
                error,
            )
        ...
        for row in chunk:
            ...
            is_census = enriched.get("geocode_source") == geocoding.SOURCE_CENSUS
            cache.put(
                unique_key,
...
def _print_stats(path, counters):
    total = counters["state"] + counters["skipped_no_address"] + counters["candidates"]
    geocoded = counters["geocoded_exact"] + counters["geocoded_approx"]
    unmatched = counters["unmatched_tie"] + counters["unmatched_no_match"] + counters["query_failed"]
...
def build_arg_parser():
    ...
    parser.add_argument(
        "inputs", nargs="+", help="state output file(s) to enrich; JSON or CSV, detected by the .json/.csv extension"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument("--cache", default=DEFAULT_CACHE_PATH, help="SQLite cache path (default: %(default)s)")
    parser.add_argument("--no-cache", action="store_true", help="ignore the cache (do not read or write it)")
    parser.add_argument(
        "--benchmark", default=geocoding.CENSUS_BENCHMARK, help="Census benchmark (default: %(default)s)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--timeout", type=int, default=DEFAULT_TIMEOUT, help="per-request timeout seconds (default: %(default)s)"
    )
    parser.add_argument(
        "--max-retries", type=int, default=DEFAULT_MAX_RETRIES, help="retries per chunk (default: %(default)s)"
    )
    parser.add_argument(
        "--limit", type=int, help="cap the number of unique addresses queried (for testing on large files)"
    )
    parser.add_argument("--dry-run", action="store_true", help="partition and report only; no network, no write")
