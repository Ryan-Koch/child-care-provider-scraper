...
class UtahSpider(scrapy.Spider):
    ...
    def generate_search_request(self, zip_code, page):
        ...
        payload = {"andPredicates": {"filters": [{"fieldName": "zipCode", "operator": ":", "value": zip_code}]}}

        return scrapy.Request(
            url,
            method="POST",
            body=json.dumps(payload),
            headers={"Content-Type": "application/json", "Referer": "https://jobs.utah.gov/"},
    ...
    def parse_search(self, response):
        ...
        self.logger.info(
            f"Found {len(providers)} providers in zip {response.meta['zip_code']} page {response.meta['page']}"
        )
        ...
        for provider in providers:
            ...
            if program_id:
                ...
                yield scrapy.Request(detail_url, callback=self.parse_detail, meta={"search_data": provider})
    ...
    def parse_detail(self, response):
        ...
        item["ut_vacancies"] = (
            detail_data.get("vacancies") if "vacancies" in detail_data else search_data.get("vacancy")
        )
