...
class NewMexicoSpider(scrapy.Spider):
        ...
        try:
            ...
            while True:
                ...
                if await show_more_button.is_visible():
                    self.logger.info(
                        f"Current results count: {current_count}. Clicking 'Show More Results' (Click {clicks + 1})..."
                    )
                    ...
                    try:
                        await page.wait_for_function(
                            f"document.querySelectorAll('.listing-card').length > {current_count}", timeout=15000
                        )
                        consecutive_no_increase = 0
                    except Exception as e:
            ...
            for link in unique_links:
                yield scrapy.Request(
                    response.urljoin(link), callback=self.parse_detail, meta={"source_state": "New Mexico"}
                )
    ...
    def parse_detail(self, response):
        ...
        provider["administrator"] = (
            response.xpath('//div[div[contains(., "Program Director")]]/div[contains(@class, "text-lg")]/text()')
            .get("")
            .strip()
        )
        ...
        # Address
        address_div = response.xpath(
            '//div[@id="location"]//div[contains(@class, "flex-col") and contains(@class, "pl-12")]'
        )
        ...
        # NM Star Level
        star_level = "".join(
            response.xpath('//div[@id="network_rating"]//text()[contains(., "Star Level")]').getall()
        ).strip()
        ...
        ages_banner = (
            response.xpath('(//div[contains(@id, "availablity-banner")]//text()[contains(., "enrollments")])[1]')
            .get("")
            .strip()
        )
