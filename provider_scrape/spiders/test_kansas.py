...
def test_parse_counties_extracts_all_and_posts_one_search_per_county(spider):
    ...
    assert '"selectedIndices": [1, 2, 3, 4, 5, 6, 7]' in \
        formdata[PROGRAM_TYPE_CLIENTSTATE_FIELD]
...
def test_parse_results_full_page_mints_and_chains_next(spider):
    response = _response("ks_results_page.html",
                          meta={"county": "Crawford", "page": 1})
...
def test_parse_results_short_page_stops(spider):
    response = _response("ks_results_last_page.html",
                          meta={"county": "Crawford", "page": 6})
    outputs = list(spider.parse_results(response))
    mints, nexts = split_requests(outputs)
...
def test_dedupe_does_not_affect_the_pagesize_stop_rule(spider):
    ...
    # request.
    response2 = _response("ks_results_page.html",
                           meta={"county": "Crawford", "page": 2})
    second_mints, second_nexts = split_requests(
        list(spider.parse_results(response2)))
    assert second_mints == []
...
def test_parse_detail_golden(spider):
    row = {"token": "SearchLink.abc==", "city": "Dodge City", "zip": "67801",
           "county": "Ford", "program_type": "School Age Program"}
...
def test_parse_detail_suppressed(spider):
    row = {"token": "SearchLink.sup==", "city": "Topeka", "zip": "66611 - 0000",
           "county": "Shawnee", "program_type": "Family Child Care Home"}
...
def test_program_type_disagreement_prefers_detail(spider):
    row = {"token": "SearchLink.comp==", "city": "Wichita", "zip": "67205",
           "county": "Sedgwick", "program_type": "Preschool"}
...
def test_empty_grid_yields_no_items(spider):
    ...
    row = {"token": "SearchLink.abc==", "city": "Dodge City", "zip": "67801",
           "county": "Ford", "program_type": "School Age Program"}
    center_item = next(spider.parse_detail(
        _response("ks_detail_center.html", meta={"row": row})))
    assert not any(i["type"] == "Complaint Survey"
                   for i in center_item["inspections"])

    row2 = {"token": "SearchLink.comp==", "city": "Wichita", "zip": "67205",
            "county": "Sedgwick", "program_type": "Child Care Center"}
    complaints_item = next(spider.parse_detail(
        _response("ks_detail_complaints.html", meta={"row": row2})))
    assert not any(i["type"] == "Administrative Order"
                   for i in complaints_item["inspections"])
...
def test_multi_nosf_survey_yields_two_items(spider):
    ...
    shared_survey = [i for i in item["inspections"]
                     if i.get("ks_survey_number") == "26-004836"]
...
@pytest.mark.parametrize("raw,city,expected_address,expected_street", [
    # plain glued street+city
    ("240 San Jose Dr Dodge City, KS 67801", "Dodge City",
     "240 San Jose Dr, Dodge City, KS 67801", "240 San Jose Dr"),
    # unit letter glued directly to the city with no space at all
    ("601 S Edwards St Unit ALewis, KS 67552", "Lewis",
     "601 S Edwards St Unit A, Lewis, KS 67552", "601 S Edwards St Unit A"),
    # -0000 filler dropped
    ("1831 E 21st St Andover, KS 67002-0000", "Andover",
     "1831 E 21st St, Andover, KS 67002", "1831 E 21st St"),
    # city name equals the county name -- must anchor on the ZIP tail, not
    # search for the city string anywhere in the address.
    ("612 N Commercial Ave Sedgwick, KS 67135", "Sedgwick",
     "612 N Commercial Ave, Sedgwick, KS 67135", "612 N Commercial Ave"),
    # real ZIP+4 preserved
    ("7026 W 21st St N Wichita, KS 67205-1760", "Wichita",
     "7026 W 21st St N, Wichita, KS 67205-1760", "7026 W 21st St N"),
])
...
def test_parse_detail_missing_name_emits_nothing(spider, caplog):
    ...
    assert items == []
    assert any("parse_detail missing faciltyNameValue" in record.message
