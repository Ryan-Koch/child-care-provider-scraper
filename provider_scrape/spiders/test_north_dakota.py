...
def detail_response(profile, pid=None):
    ...
    return TextResponse(url=req.url, body=json.dumps(profile).encode(), encoding="utf-8", request=req)


def search_response(results, lat=47.5, lon=-100.0, half_mi=6.0, depth=0):
    body = json.dumps({"results": results, "limit": 100, "pageSize": 20, "sort": "distance"})
    req = Request(SEARCH_URL, method="POST", meta={"lat": lat, "lon": lon, "half_mi": half_mi, "depth": depth})
    return TextResponse(url=SEARCH_URL, body=body.encode(), encoding="utf-8", request=req)


def make_results(n, distance, start=0):
    return [
        {"id": f"id{start + i:04d}", "orgName": f"Program {start + i}", "locationFilterDistance": distance}
        for i in range(n)
    ]
...
def test_parse_detail_golden(spider):
    ...
    assert item["provider_url"].endswith("(slide-full:68484324fa1ed3f5f772600a/profile)")
    # nd_ fields
    assert item["nd_quality_rating"] == "Step 1"
    assert item["nd_total_vacancies"] == 0
    assert len(item["nd_vacancies_by_age"]) == 4
    assert item["nd_vacancies_by_age"][0] == {"age_group": "Infant (0-17 months)", "vacancies": 0}
...
def test_parse_detail_minimal(spider):
    item = next(spider.parse_detail(detail_response({"id": "abc123", "orgName": "Tiny Home Daycare"})))
...
def test_status_deactivated(spider):
    item = next(spider.parse_detail(detail_response({"id": "x", "orgName": "Gone", "deactivated": True})))
...
@pytest.mark.parametrize(
    "total,expected_key,expected_val",
    [
...
def test_accepting_new_children(spider, total, expected_key, expected_val):
    item = next(spider.parse_detail(detail_response({"id": "x", "orgName": "n", "totalVacancies": total})))
...
def test_age_group_booleans_partial(spider):
    ...
    item = next(spider.parse_detail(detail_response({"id": "x", "orgName": "n", "ageGroupsServed": [1, 4]})))
...
def test_search_saturated_densifies(spider):
    ...
    outputs = list(spider.parse_search(search_response(make_results(100, 3.0), half_mi=6.0, depth=0)))
    details, searches = split_requests(outputs)
...
def test_search_saturated_but_covered_no_densify(spider):
    ...
    outputs = list(spider.parse_search(search_response(make_results(100, 9.0), half_mi=6.0, depth=0)))
    details, searches = split_requests(outputs)
    assert len(details) == 100
    assert len(searches) == 0
...
def test_search_depth_cap_stops_densifying(spider):
    outputs = list(spider.parse_search(search_response(make_results(100, 1.0), half_mi=4.0, depth=MAX_DENSIFY_DEPTH)))
