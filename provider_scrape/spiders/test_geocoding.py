...
def test_has_coordinates_true_when_both_present():
    assert geocoding.has_coordinates({"latitude": "39.78", "longitude": "-89.65"}) is True


def test_has_coordinates_false_when_one_missing():
    assert geocoding.has_coordinates({"latitude": "39.78", "longitude": None}) is False


def test_has_coordinates_false_on_empty_strings():
    assert geocoding.has_coordinates({"latitude": "  ", "longitude": ""}) is False
...
def test_split_uses_parsed_components_and_peels_street():
    ...
    assert geocoding.split_address_for_geocode(item) == ("123 Main St", "Springfield", "IL", "62704")


def test_split_falls_back_to_full_address_when_unparsed():
    item = {"address": "123 Main St Apt 4"}
    assert geocoding.split_address_for_geocode(item) == ("123 Main St Apt 4", "", "", "")
...
def test_build_batch_row_prefixes_id():
    ...
    assert geocoding.build_batch_row(7, item) == ["7", "123 Main St", "Springfield", "IL", "62704"]
