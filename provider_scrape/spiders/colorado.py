...
class ColoradoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for result in results:
            ...
            item["provider_type"] = (
                result.xpath('.//p[strong[contains(text(),"Care Setting")]]/text()[normalize-space()]').get("").strip()
            )
            ...
            item["co_infant_openings"] = (
                result.xpath('.//p[strong[contains(text(),"Infant Openings Available")]]/text()[normalize-space()]')
                .get("")
                .strip()
            )
            item["co_toddler_openings"] = (
                result.xpath('.//p[strong[contains(text(),"Toddler Openings Available")]]/text()[normalize-space()]')
                .get("")
                .strip()
            )
            item["co_preschool_openings"] = (
                result.xpath('.//p[strong[contains(text(),"Preschool Openings Available")]]/text()[normalize-space()]')
                .get("")
                .strip()
            )
            item["co_school_age_openings"] = (
                result.xpath('.//p[strong[contains(text(),"School Age Openings Available")]]/text()[normalize-space()]')
                .get("")
                .strip()
            )
        ...
        if next_link and view_state:
            ...
            if match:
                ...
                form_data = {"com.salesforce.visualforce.ViewState": view_state}
                ...
                yield scrapy.FormRequest.from_response(
                    response, formid="page:searchForm", formdata=form_data, dont_click=True, callback=self.parse
                )
    ...
    def parse_detail(self, response):
        ...
        # License Number
        item["license_number"] = (
            response.xpath('//p[strong[contains(text(),"License Number")]]/text()[normalize-space()]').get("").strip()
        )
        ...
        item["co_accepting_new_children"] = response.xpath(
            '//p[strong[contains(text(),"Accepting New Children")]]/span/text()'
        ).get()

        # Capacity
        item["capacity"] = (
            response.xpath('//p[strong[contains(text(),"Capacity")]]/text()[normalize-space()]').get("").strip()
        )
        ...
        # Licensed to Serve
        item["co_licensed_to_serve"] = (
            response.xpath('//p[strong[contains(text(),"Licensed to Serve")]]/text()[normalize-space()]')
            .get("")
            .strip()
        )
        ...
        special_needs = response.xpath(
            '//div[contains(@class,"field-name-field-info") and contains(.,"Special Needs")]/text()'
        ).getall()
        ...
        # License Type
        item["co_license_type"] = (
            response.xpath('//p[strong[contains(text(),"License Type")]]/text()[normalize-space()]').get("").strip()
        )

        # License Issue Date
        item["co_license_issue_date"] = response.xpath(
            '//p[strong[contains(text(),"License Issue Date")]]/span/text()'
        ).get()
