...
def county_response(spider, county="Pickett", fixture="tn_county_pickett.json"):
    ...
    return TextResponse(url=req.url, body=_load(fixture).encode(), encoding="utf-8", request=req, status=200)
...
def detail_response(sysid, fixture, meta=None):
    ...
    return TextResponse(url=url, body=_load(fixture).encode(), encoding="utf-8", request=req, status=200)
...
def test_flatten_hours():
    hours = [{"name": "Monday", "hours": ["06:30 AM -- 05:30 PM"]}, {"name": "Saturday", "hours": []}]
...
def test_build_ages_served():
    assert build_ages_served("6 Week(s)", "12 Year(s)") == "6 Week(s) to 12 Year(s)"
...
def test_visit_report_url_trims_cruft():
    raw = (
        "?id=cp_visit_details_maps&sysId1=abc123&name1=null&parent=X"
        "&provList=?id=cp_provider_search_results&visitsListURL=?id=y"
    )
    assert visit_report_url(raw) == "https://onedhs.tn.gov/csp?id=cp_visit_details_maps&sysId1=abc123"
    assert visit_report_url(None) is None
    # No sysId1 -> fall back to absolutizing the relative source URL.
    assert visit_report_url("?id=cp_monitoring_visits").startswith("https://onedhs.tn.gov/csp?id=cp_monitoring_visits")
...
def test_parse_detail_rated(spider):
    item = next(spider.parse_detail(detail_response("a1e297b5dbc10110c21143d913961921", "tn_detail_rated.json")))
    ...
    assert item["inspections"][0]["report_url"].startswith("https://onedhs.tn.gov/csp?id=cp_visit_details_maps&sysId1=")
    assert item["provider_url"].endswith("sysid=a1e297b5dbc10110c21143d913961921")
...
def test_parse_detail_unrated(spider):
    item = next(spider.parse_detail(detail_response("12c04846dbdf0590ee34266e139619d5", "tn_detail_unrated.json")))
    assert item["provider_type"] == "DOE"
    # No rating block for an unrated provider.
    for absent in ("tn_quality_rating", "tn_rating_scorecard", "tn_rating_effective_date"):
