...
class OhdcySpider(Spider):
    ...
    def after_submit(self, response):
        ...
        if max_pages:
            ...
            match = re.search(
                r"&p=(\d+)$", max_pages_href
            )  # Should pull number at the end of the href string after the &p=
            if match:
        ...
        for row in inspection_rows:
            ...
            if len(columns) >= 6:  # Ensure enough columns are present
                ...
                inspection["corrective_status"] = (
                    columns[3]
                    .xpath('./span[@id="statusDescription"]/text()')
                    .get(default="")
                    .strip()
                    .replace("\r\n", " ")
                )
